...
// {
//     float rad_per_step = M_2PI / 1000.0f;     // 整步
//     float motor_rad_s = motor_fre * rad_per_step;       // Hz → rad/s
//     float _error_speed = _target_speed - Angular_velocity_final;
//     float _delta_rad = pid_speed.update(_error_speed);  // rad/s 增量
//     motor_fre += _delta_rad / rad_per_step;             // rad/s → Hz
    
//     if(motor_fre >= FV_MAX) motor_fre = FV_MAX;
//     if(motor_fre <= -FV_MAX) motor_fre = -FV_MAX;
//     setFre();
// }
